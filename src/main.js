...
const regexPatterns = {
  ...
  aluEmailSpecificRegex:
    /[^\s@.?*|{}\[\]()"\\<>';%=$]+(\.[^\s@.?*|{}\[\]()"\\<>';%=$]+)*@(alueducation\.com|alumni\.alueducation\.com|si\.alueducation\.com)(?![\w.-])/g,
...
async function regexOnboarding(path) {
  try {
    // Each line from the input file is read and matched against each regex pattern
    const line = createInterface({
      input: createReadStream(path),
      crlfDelay: Infinity,
    });
    line.on("line", (line) => {
      // Match each line against the email regex
      // If a match is found, it is added to the validEmails array
      const matchingEmails = line.match(regexPatterns.emailRegex);
      if (matchingEmails) {
        validEmails.push(...matchingEmails);
      }
      // Match each line specifically against the ALU email regex
      // If a match is found, it is added to the validALUEmails array
      ...
      // Match each line against the credit card regex
      // If a match is found, it is added to the validCreditCards array
      ...
      // Match each line against the phone number regex
      // If a match is found, it is added to the validPhoneNumbers array
      ...
      // Match each line against the time format regex
      // If a match is found, it is added to the validTimeFormats array
    ...
    line.on("close", () => {
      // Log the match results to the console
      ...
      // Write all the match results to the output file
      writeFileSync(
        outputFile,
